...
def check_scores():
    with get_session() as session:
        ...
        # Summarized
        summarized = session.query(func.count(Article.id)).where(Article.processed_content.is_not(None)).scalar()
        # Rated
        rated = session.query(func.count(Article.id)).where(Article.impact_score.is_not(None)).scalar()
        # Non-null but zero?
        ...
        if rated > 0:
            statement = select(Article.impact_score).where(Article.impact_score.is_not(None))
