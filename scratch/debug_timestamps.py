...
with get_session() as session:
    ...
    if last_fetched:
        count_last_24 = session.exec(
            select(func.count(Article.id)).where(Article.fetched_at >= (last_fetched - timedelta(hours=24)))
        ).one()
        ...
        for profile in profiles:
            count = session.exec(
                ...
                .where(Article.feed_profile == profile)
                .where(Article.fetched_at >= (last_fetched - timedelta(hours=24)))
            ).one()
